...
getRCOSetSettings <- function(RCOSetID){
...
get_ap_bb_ticker_by_name <- function(portfolioName){
...
getTargetTable <- function(portfolioName, calculation_date, isReal){
...
f.setOptfunctions <- function(set, k = 0){
  ...
  if(set$IndexFlex == TRUE)
  {
    ...
    # so it's not relevant for objective function.
...
f.getWeightRange <- function(lb,
  ...
  # tradeability is quoted in days/10bp
  t_restrict <- LowConvictionExitInDays / tradeability * ConvictionDegree *10*10^(-4)
...
OptimizationResultsCharacteristics <- function(rw, cov, lb, ub, set, rb_target){
  ...
  # Check that portfolio tracking error is not changed
  portf_te_impl <- as.numeric(sqrt(t(rw_impl) %*% as.matrix(cov) %*% rw_impl))
  if(round(te,4) != round(portf_te_impl, 4))
  {
...
f.runRCO <- function(targ_df,set_df,cov_df, MaxAttempts = 10){
  ...
  if(is.data.frame(targ_df) == TRUE &&
    ...
    if(all(targ_df$RiskBudget >= 0) &&
      ...
      # assert that targ_df still fits COV!
      ...
      while(runAnotherOptAttempt)
      {
        ...
        if(opt_output$status >= 0){
          ...
          if(all(check_sign_v > 0)){
            ...
            print(paste("The ", attempt - 1, " violates direction constraints!",
...
runRCOFromAPS <- function(portfolioName,
  ...
  if(any((target_table$lb >= 0 & target_table$conviction < 0) == FALSE) &&
    ...
    if(optimization_result_l$optim_details$ValidResult){
      ...
      stopifnot(is.null(names(optimization_result_l$risk_contributions_v)) == FALSE)
      stopifnot(is.null(names(risk_contribution_v[stocks_with_signal_ind])) == FALSE)
      stopifnot(names(optimization_result_l$risk_contributions_v) == names(risk_contribution_v[stocks_with_signal_ind]))
      risk_contribution_v[stocks_with_signal_ind] <- optimization_result_l$risk_contributions_v
...
runRCOLoops <- function(Portfolio,                 #character string of the portfolio. used to identify the targ-file to load
                        ...
                        InputParameters = "targ_from_DB",         #character string like fitting the variable input in the specified targ-sheet
  ...
  read_target_settings_from_excel <- function(file_path_in, input_file, ws_name){
    ...
    tryCatch({
      ...
      target_settings_df <- na.omit(target_settings_df)
    },
    error = function(e){
      print(e)
      stop(e)
  ...
  if(length(def_cov_run_id_ind) > 0){
    ...
    if(any(is.na(cov_run_id_v[def_cov_run_id_ind]))){
      print("runRCOLoops: There is no default covariance matrix for ", Portfolio)
      stop("runRCOLoops: There is no default covariance matrix for ", Portfolio)
    }else{
      # Nothing it is ok
    }
  }else{
  ...
  for(k in 1:valid_settings_number){
    ...
    if(k == 1){
      ...
      if(targ_from_excel == TRUE){
        ...
        # Target table should be retrieved from DB
        target_settings_df <- getTargetTable(portfolioName = Portfolio,
                                             calculation_date = calculation_date,
                                             isReal = FALSE)
    ...
    # Update risk contributions list
    risk_contributions_l[[k]] <- RCOthisLoop$risk_contributions_v
    ...
    RCOres_l <- list(settings_df = valid_settings_df,
                     ...
                     risk_contributions_l = risk_contributions_l)

    output_file_name <- paste(file_path_out, Sys.Date(), " RCOresPAA.R", sep = "")
    save(RCOres_l, file = output_file_name)
  }
  print(paste("all", valid_settings_number ,"loops completed. Storing of results in process"))

  # Create names like "set1", "set2" etc for a RC list
  names_v <- sapply(seq(1:valid_settings_number), FUN = function(x){paste0("set", x)})
  names(risk_contributions_l) <- names_v
...
compareRCOruns <- function(setid,
  ...
  if(xaxis %in% available_x_values_v &&
    ...
    if(length(group_by) > 1){
      ...
      full_names_v <- available_x_values_v[available_x_values_v %in% group_by]
      short_names_v <- names(full_names_v)
      group_by_cols_df <- opt_res_df[,full_names_v, drop = FALSE]
      n <- nrow(group_by_cols_df)
      group_by_values_v <- rep("", n)
      for(i in 1:n)
      {
        values_v <- group_by_cols_df[i,]
        group_by_value <- paste(short_names_v, values_v, sep=':', collapse = ';')
        group_by_values_v[i] <- group_by_value
      }

      opt_res_df$GroupBy <- as.factor(group_by_values_v)
    ...
    ggplot2::ggplot(data = opt_res_df,
                    ggplot2::aes_string(y = yaxis,
                                        ...
                                        color = ifelse(length(group_by) == 1, group_by, "GroupBy"),
                                        group = ifelse(length(group_by) == 1, group_by, "GroupBy"),
                                        fill = ifelse(length(group_by) == 1, group_by, "GroupBy"))) +
...
plotSingleSetIDfromRCOres <- function(setid, RCOres_l, plot_type)
{
  ...
  risk_contributions_v <- RCOres_l$risk_contributions_l[[setid]]
...
draw_plot <- function(rw_v,
  ...
  adjust_range <- function(lb_change_mult,
                           ub_change_mult,
                           values_v,
                           chart_type){

    # The function abjust xlim and ylim based on sign
    # is used in plots

    if(lb_change_mult > 0 && lb_change_mult < 1 &&
       ub_change_mult > 0 && ub_change_mult < 1){

      minv <- min(values_v)
      maxv <- max(values_v)

      if(minv > 0){
        # Ex: min = 0.4, mult = 0.1 --> 0.4 * 0.9 = 0.36 (new lower bound 10 % lower)
        minv <- minv * (1 - lb_change_mult)
      }else{
        # minv <= 0
        # Ex: min = -0.4, mult = 0.1 --> -0.4 * 1.1 = -4.4 (new lower bound 10 % lower)
        minv <- minv * (1 + lb_change_mult)
      }

      if(maxv > 0){
        # Ex: max = 0.4, mult = 0.1 --> 0.4 * 1.1 = 4.4 (new upper bound 10 % bigger)
        maxv <- maxv * (1 + ub_change_mult)
      }else{
        # maxv <= 0
        # Ex: max = -0.4, mult = 0.1 --> -0.4 * 0.9 = -0.36 (new upper bound 10 % bigger)

        maxv <- maxv * (1 - ub_change_mult)
      }

      # Bar plot is tricky. It plots always from 0
      if(chart_type == 'bar' && maxv > 0 && minv > 0){
        minv <- 0
      }else if(chart_type == 'bar' && maxv < 0 && minv < 0){
        # some additional space for legend is required in this case
        # 50% of graph should be ok
        maxv <- 0 + (maxv - minv) * 0.5
      }else{
      ...
      stop("adjust_range: input params must be between 0 and 1")
    }
  }
  ...
  if(all(names_targ_v %in% colnames(targ_input_df)) == TRUE &&
    ...
    all_v <- c(rb_act_v, rc_act_v)

    # Add 10% to xlim and ylim to let text be inside a plot
    range <- adjust_range(lb_change_mult = 0.1,
                          ub_change_mult = 0.1,
                          values_v = all_v,
                          chart_type = 'bar')
    ...
    if(any(is.na(rw_v))){
      ...
      if(plot_type == "overview"){
        ...
        plot_active_relative_weights <- function(rw_act_v,
          ...
          range <- adjust_range(lb_change_mult = 0.2,
                                ub_change_mult = 0.6,
                                values_v = rw_act_v,
                                chart_type = 'bar')
...
update_default_settings <- function(pname, setID, covID){

  # Description:
  # The function updates default settings for a portfolio
  # This is done using SP

  # Dependensies
  #   library(RODBC)
  #   SP:  [betsizing].[uspUpdateCalculationDefaultSettings]

  if(is.null(pname) == FALSE &&
     is.null(setID) == FALSE &&
     is.null(covID) == FALSE &&
     length(pname) == 1 &&
     length(setID) == 1 &&
    ...
    if(is.character(pname) && is.numeric(setID) && is.numeric(covID)){

      query <- paste0("EXEC [betsizing].[uspUpdateCalculationDefaultSettings] ",
                      "@CustomPortfolioName = '",pname,"', ",
                      "@FKCalculationRCOSettingSetID = '",setID,"', ",
                      "@CovMatrixRunID = '",covID,"'")

      con <- FAFunc.GetDB()
      query_res <-  RODBC::sqlQuery(con, query)
      RODBC::odbcClose(con)

      if(class(query_res) == "character"){
        if(length(query_res) == 0){

          print("Default settings have been updated!")
        }else{
          print("update_default_settings:exec [betsizing].[uspUpdateCalculationDefaultSettings]  gave no results")
          stop("update_default_settings:exec [betsizing].[uspUpdateCalculationDefaultSettings]  gave no results")
        }
      }else{
        print("update_default_settings:SQL query failed")
        stop("update_default_settings:SQL query failed")
      }
    }else{
      print("update_default_settings: setID and covID must be numbers")
      stop("update_default_settings: setID and covID must be numbers")
    }
  }else{
    print("update_default_settings: some input parameters are empty or length != 1")
    stop("update_default_settings: some input parameters are empty  or length != 1")
  }
}

# get search help covariance run Ids from Data base
get_sh_cov_run_ids <- function(portfolio_name){
  # Desctiption:
  #   This function takes recent covariance run IDs from DB
  #   Additionally default run ID is displayed

  # Depandencies:
  #   betsizing.uspCheckPortfolioBetsizingStatusCovMa
  #   RODBC

  if(is.character(portfolio_name) && length(portfolio_name == 1)){
    ...
    if(class(result_df) == "data.frame"){
      if(nrow(result_df) > 0){
        ...
        req_names <- c("RunId", "IsDefaultCovMa")
        if(all(req_names %in% colnames(result_df))){
          # if it is default matrix add this info to a run ID
          sh_values_v <- ifelse(test = (result_df$IsDefaultCovMa == TRUE),
                                yes = paste0(result_df$RunId, " (default)"),
                                no = result_df$RunId)

          # Find default cov ma index
          def_cov_ind <- grep(pattern = "(default)",
                              x = sh_values_v,
                              fixed = TRUE)

          if(length(def_cov_ind) == 1){
      ...
      # class(result_df) != "data.frame")
      print(paste0("NLopt solver status: ", -100))
      print(paste0("NLopt solver status message: get_sh_cov_run_ids: Select statement contains an error"))
      stop("get_sh_cov_run_ids: Select statement contains an error ")
    }
  }else{
    print("get_sh_cov_run_ids: portfolio name parameter has wrong type")
    stop("get_sh_cov_run_ids: portfolio name parameter has wrong type")
  }
}

# get search help values for Excel worksheets
get_sh_ws <- function(portfolio_name, date){
  ...
  if(is.character(portfolio_name) &&
    ...
    if(portfolio_name == "RE_EU"){
      ...
      input_file <- paste("bet_targets_", portfolio_name,".xlsx",sep="",collapse="")
      path <- paste(folder_path, input_file, sep = "", collapse = "")
    }
...
get_sh_portfolio_name <- function(){

  # Description:
  #    The function collects all available analyst portfolio names
  #    from DB. This list of names is used in portfolio name SH in R shiny

  # Depandencies:
  #   RODBC
  #   aktienmodell.config.CustomPortfolios

  # query
  query <- paste0("SELECT [Name] FROM aktienmodell.config.CustomPortfolios ",
                  "WHERE [Name] like 'AP%'")
  ...
  if(is.data.frame(result_df)){
    if(nrow(result_df) > 0){
      ...
      if(all(req_names %in% colnames(result_df))){
        # if it is default matrix add this info to a run ID
        sh_values_v <- result_df$Name

        return(sh_values_v)

      }else{
        print("get_sh_portfolio_name: Output data frame structure is not correct")
        stop("get_sh_portfolio_name: Output data frame structure is not correct")
      }
    }else{
      print(paste0("NLopt solver status: ", -100))
      paste0("NLopt solver status message: there are no portfolios in the system")
      stop("NLopt solver status message: there are no portfolios in the system")
    }
  }else{
    # class(result_df) != "data.frame")
    print(paste0("NLopt solver status: ", -100))
    print(paste0("NLopt solver status message: get_sh_portfolio_name: Select statement contains an error"))
    stop("get_sh_portfolio_name: Select statement contains an error ")
  }
}


plot_weights_dev_overview <- function(weights_ma){
  # Description:
  #   The function can be used to easily compare the change in
  #     weights by runs
  ...
  if(is.matrix(weights_ma)){

    df <- as.data.frame(weights_ma)
    df$tickers <- row.names(df)

    df <- as.data.frame(reshape2::melt(data = df,
                                       id.vars = 'tickers',
                                       value.name = "WeightRel",
                                       variable.name = c("RunID")))

    ggplot2::ggplot(df,
                    ggplot2::aes(x=tickers,
                                 y=WeightRel,
                                 fill = RunID
                                 )) +
      #fill=rownames(as.data.frame(t(weights_ma)))
      ggplot2::geom_bar(stat="identity",position=ggplot2::position_dodge()) +

      ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 90, hjust = 1)) +

      ggplot2::labs(title = "Relative weights by runs")

  }else{
    print("plot_weights_dev_overview: input param has wrong type")
    stop("plot_weights_dev_overview: input param has wrong type")
  }
}

plot_rc_dev_overview <- function(rc_l){
  # Description:
  #   The function can be used to easily compare the change in
  #     risk contributions by runs

  # Dependenies:
  # library(ggplot2)
  # library(reshape2)

  if(is.list(rc_l) && is.null(names(rc_l)) == FALSE){


    df <- as.data.frame(rc_l)
    df$tickers <- row.names(df)
    ...
    ggplot2::labs(title = "Risk Contributions by runs")

  }else{
    print("plot_rc_dev_overview: input param has wrong type")
    stop("plot_rc_dev_overview: input param has wrong type")
  }
}
