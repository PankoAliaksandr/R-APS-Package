...
f.writeOptdetails2xlsx <- function(RCOres_l,
  ...
  stopifnot("setID" %in% colnames(settings_df))
...
runRCOLoops <- function(Portfolio,                 #character string of the portfolio. used to identify the targ-file to load
  ...
  read_target_settings_from_excel <- function(file_path_in, input_file, ws_name){
    ...
    if(class(target_settings_df) == "data.frame" &&
      ...
      if(all(colnames(target_settings_df) %in% targ_col_names_v)){
        ...
        if(length(unique(target_settings_df$Ticker)) == nrow(target_settings_df$Ticker)){
  ...
  get_cov_ma <- function(cov_id = NULL,
                         portfolio,
                         cov_ret,
                         calc_method,
                         sec_names){
    ...
    cov <- f.matchCOVtoTickers (Tickers = sec_names, COV = as.data.frame(cov))
  ...
  optim_details_names_v <- list(c("Calculated","TEact-TEtarg","%weight_limits_hit",
  ...
  for(k in 1:valid_settings_number){
    ...
    if(k == 1){
      ...
      sec_names_v <- target_settings_df$Ticker
    ...
    cov_active <- f.matchCOVtoTickers (Tickers = rownames(targ_active), COV = cov_ma)
